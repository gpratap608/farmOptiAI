...
const Services = () => {
    ...
    const dataShow = async (e) => {
        ...
        try {
            const response_ML_urea = await axios.get(`https://192.168.3.76:8080/urea/${cropNo}/${landArea}`)
            setUrea(Number(response_ML_urea.data.prediction))
            const response_ML_phosphate = await axios.get(`https://192.168.3.76:8080/phosphate/${cropNo}/${landArea}`)
            setPhosphate(Number(response_ML_phosphate.data.prediction))
            const response_ML_potash = await axios.get(`https://192.168.3.76:8080/potash/${cropNo}/${landArea}`)
